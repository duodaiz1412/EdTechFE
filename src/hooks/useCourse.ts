...
import { CourseService, ICourseRequest, ICourse, IChapterRequest, ILessonRequest } from '@/lib/services/course.services';
...
export interface UseCourseReturn {
  ...
  getMyCourses: (page?: number, size?: number, status?: string) => Promise<ICourse[]>;
...
const useCourse = (): UseCourseReturn => {
  ...
  // Intended learners data
  const [intendedLearners, setIntendedLearners] = useState<IntendedLearnersData>({
    learningObjectives: [],
    requirements: [],
    targetAudience: [],
  });
  ...
  // Course operations
  const createCourse = useCallback(async (formData?: any): Promise<string | null> => {
    try {
      setLoading(true);
      clearError();
  ...
  const loadCourse = useCallback(async (courseId: string): Promise<void> => {
    try {
      ...
      const response = await CourseService.getCourseForInstructor(courseId, accessToken);
      setState(prev => ({ ...prev, course: response.data }));
    } catch (err) {
  ...
  const getMyCourses = useCallback(async (page: number = 0, size: number = 10, status?: string): Promise<ICourse[]> => {
    try {
      setLoading(true);
      clearError();
      ...
      const response = await CourseService.getMyCourses(accessToken, page, size, status);
  ...
  const updateCourse = useCallback(async (courseId: string, data: ICourseRequest): Promise<boolean> => {
    try {
      setLoading(true);
      clearError();
      ...
      const response = await CourseService.updateCourse(courseId, data, accessToken);
      setState(prev => ({ ...prev, course: response.data }));
      return true;
    } catch (err) {
  ...
  const publishCourse = useCallback(async (courseId: string): Promise<boolean> => {
    try {
      setLoading(true);
      clearError();
  ...
  // Chapter operations
  const createChapter = useCallback(async (courseId: string, data: IChapterRequest): Promise<boolean> => {
    try {
      setLoading(true);
      clearError();
  ...
  const updateChapter = useCallback(async (chapterId: string, data: IChapterRequest): Promise<boolean> => {
    try {
      setLoading(true);
      clearError();
  ...
  // Lesson operations
  const createLesson = useCallback(async (chapterId: string): Promise<string | null> => {
    try {
      setLoading(true);
      clearError();
      ...
      const response = await CourseService.createLesson(chapterId, accessToken);
  ...
  const updateLesson = useCallback(async (lessonId: string, data: ILessonRequest): Promise<boolean> => {
    try {
      setLoading(true);
      clearError();
  ...
  const updateIntendedLearners = useCallback((data: Partial<IntendedLearnersData>) => {
