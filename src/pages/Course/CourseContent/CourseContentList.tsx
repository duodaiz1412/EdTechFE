...
export default function CourseContentList({curriculum}: {curriculum?: any[]}) {
  return (
        ...
        >
          ...
          <div className="collapse-title bg-slate-200 space-x-2 flex items-center">
            <h2 className="font-semibold">
              Section {index + 1}: {chapter.chapterName}
            </h2>
            ...
            <div className="pt-3 divide-solid divide-y">
              {chapter.lessons ? (
                chapter.lessons.map((lesson) => (
                  <CourseContentItem key={lesson.title} lesson={lesson} />
                ))
              ) : (
