...
export default function PreviewCourse() {
  ...
  useEffect(() => {
    const loadCourseData = async () => {
      ...
      try {
        ...
        const response = await instructorServices.getCourseForInstructor(courseId, accessToken);
        if (response.data) {
  ...
  useEffect(() => {
    if (lessonSlug && formData.chapters) {
      const allLessons = formData.chapters.flatMap((chapter: any) => chapter.lessons || []);
      const lesson = allLessons.find((l: any) => l.slug === lessonSlug);
      if (lesson) {
        setSelectedLesson(lesson);
        setCurrentLesson({
          chapter: formData.chapters.find((c: any) => 
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-gray-500">
                Choose a lesson from the sidebar to see how it will appear to students
              </p>
            </div>
          </div>
        )}
