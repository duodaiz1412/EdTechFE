...
const COURSE_ENDPOINTS = {
  ...
  PUBLIC_COURSE_CHAPTERS: (courseId: string) => BASE_API + `/public/courses/${courseId}/chapters`,
  PUBLIC_COURSE_REVIEWS: (courseSlug: string) => BASE_API + `/public/courses/${courseSlug}/reviews`,
  
  // Instructor endpoints
  INSTRUCTOR_COURSES: BASE_API + "/instructor/my-courses",
  INSTRUCTOR_COURSE_DETAIL: (courseId: string) => BASE_API + `/instructor/courses/${courseId}`,
  CREATE_COURSE: BASE_API + "/instructor/courses",
  UPDATE_COURSE: (courseId: string) => BASE_API + `/instructor/courses/${courseId}`,
  DELETE_COURSE: (courseId: string) => BASE_API + `/instructor/courses/${courseId}`,
  PUBLISH_COURSE: (courseId: string) => BASE_API + `/instructor/courses/${courseId}/publish`,
  
  // Chapter endpoints
  CREATE_CHAPTER: (courseId: string) => BASE_API + `/instructor/courses/${courseId}/chapters`,
  UPDATE_CHAPTER: (chapterId: string) => BASE_API + `/instructor/chapters/${chapterId}`,
  DELETE_CHAPTER: (chapterId: string) => BASE_API + `/instructor/chapters/${chapterId}`,
  
  // Lesson endpoints
  CREATE_LESSON: (chapterId: string) => BASE_API + `/instructor/chapters/${chapterId}/lessons`,
  UPDATE_LESSON: (lessonId: string) => BASE_API + `/instructor/lessons/${lessonId}`,
  DELETE_LESSON: (lessonId: string) => BASE_API + `/instructor/lessons/${lessonId}`,
  ...
  // Progress tracking
  MARK_LESSON_COMPLETED: (lessonId: string) => BASE_API + `/lessons/${lessonId}/progress`,
  GET_COURSE_PROGRESS: (courseId: string) => BASE_API + `/courses/${courseId}/my-progress`,
} as const;
...
export const courseServices = {
  ...
  async getCourseReviews(courseSlug: string, page: number = 0, size: number = 10, accessToken?: string) {
    const params = new URLSearchParams({
      page: page.toString(),
      size: size.toString(),
    });
  ...
  // Instructor APIs
  async getMyCourses(accessToken: string, page: number = 0, size: number = 10, status?: string) {
    const params = new URLSearchParams({
      page: page.toString(),
      size: size.toString(),
    });
  ...
  async updateCourse(courseId: string, courseData: ICourseRequest, accessToken: string) {
  ...
  // Chapter APIs
  async createChapter(courseId: string, chapterData: IChapterRequest, accessToken: string) {
    const response = await axios.post(
  ...
  async updateChapter(chapterId: string, chapterData: IChapterRequest, accessToken: string) {
    const response = await axios.put(
  ...
  async updateLesson(lessonId: string, lessonData: ILessonRequest, accessToken: string) {
    const response = await axios.put(
  ...
  async getMyEnrollments(accessToken: string) {
    const response = await axios.get(
      COURSE_ENDPOINTS.MY_ENROLLMENTS,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
...
export class CourseService {
  // Public methods
  static async getPublishedCourses(filters: CourseFilters = {}, accessToken?: string) {
  ...
  static async getCourseReviews(courseSlug: string, page: number = 0, size: number = 10, accessToken?: string) {
    return courseServices.getCourseReviews(courseSlug, page, size, accessToken);
  }

  // Instructor methods
  static async getMyCourses(accessToken: string, page: number = 0, size: number = 10, status?: string) {
  ...
  static async updateCourse(courseId: string, courseData: ICourseRequest, accessToken: string) {
  ...
  // Chapter methods
  static async createChapter(courseId: string, chapterData: IChapterRequest, accessToken: string) {
    return courseServices.createChapter(courseId, chapterData, accessToken);
  }

  static async updateChapter(chapterId: string, chapterData: IChapterRequest, accessToken: string) {
  ...
  static async updateLesson(lessonId: string, lessonData: ILessonRequest, accessToken: string) {
