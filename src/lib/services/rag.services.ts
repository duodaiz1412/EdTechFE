...
const RAG_ENDPOINTS = {
  ...
  ASK_V1: RAG_API_BASE_URL + "/api/v1/ask",
...
export interface AskRAGV1Request {
  question: string;
  userId: string; // Required in v1
  lessonId?: string;
  chatHistory?: ChatMessage[];
}

export interface AskRAGV1Response {
  answer: string;
  sources: (CourseSource | LessonSource)[];
}
...
export const ragServices = {
  ...
  /**
   * Ask a question to the RAG system (V1 simplified endpoint)
   */
  async askRAGV1(request: AskRAGV1Request): Promise<AskRAGV1Response> {
    const accessToken = await getAccessToken();
    ...
    try {
      const response = await axios.post<AskRAGV1Response>(
        RAG_ENDPOINTS.ASK_V1,
        request,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
